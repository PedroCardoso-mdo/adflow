...
parser.add_argument(
    "--crossflow",
    action="store_true",
    help="(sagr only) turn on the helicity crossflow term (transitionCrossflow=True) "
    "and linearize about the crossflow-on converged restart "
    "(ar5_plain_wing_sagr_crossflow_dp.cgns).",
)
...
if args.turbmodel == "sagr":
    ...
    for dv in reg_sagr.sagrAeroDVs:
        ap.addDV(dv)
    if args.crossflow:
        # crossflow-on: same case/conditions, but the D_scf helicity term is
        # active and we linearize about the crossflow-on converged state.
...
caseLabel = ("sagr/ar5" + ("/crossflow" if args.crossflow else "")) if args.turbmodel == "sagr" else args.mesh
if rank == 0:
