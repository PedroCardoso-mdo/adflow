"""Shared configuration and helpers for the SA-noft2-Gamma (SA-sgamma, 2-equation
Menter/Nichols/Lee gamma-SA with P&Z smoothing) derivative regression tests.
Derived from reg_sagr.py (2026-09-22); nw = 7 with itu1/itu2 = 6/7 = nuTilde/gamma, no reThetat.
...
sagrRestartFile = os.path.join(baseDir, "../../input_files/mdo_tutorial_sgamma_dp.cgns")
...
sagrBaseOptions = {
    ...
    # gamma, reThetat)
    "turbulencemodel": "SA-noft2-Gamma",
...
def getStateBlocks(CFDSolver):
    ...
    if nw == 7 and (nt2 - nt1) == 1:
        blocks = OrderedDict()
        blocks["meanflow"] = list(range(0, nt1 - 1))  # rho, rho*u, rho*v, rho*w, rho*E
        blocks["nuTilde"] = [nt1 - 1]  # itu1
        blocks["gamma"] = [nt1]  # itu2 (SA-noft2-Gamma: no reThetat)
    elif nw == 8 and (nt2 - nt1) == 2:
        blocks = OrderedDict()
        blocks["meanflow"] = list(range(0, nt1 - 1))  # rho, rho*u, rho*v, rho*w, rho*E
        blocks["nuTilde"] = [nt1 - 1]  # itu1
...
def assert_coupling_dot_products_allclose(handler, CFDSolver, seed=314, **kwargs):
    ...
    pairs = [
        ...
        ("meanflow", "gamma"),  # dR[gamma]/d(meanflow): Re_v, lambda_thetaL, Re_theta_c chains
