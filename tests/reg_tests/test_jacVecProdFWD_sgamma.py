...
import reg_sgamma as reg_sagr
from reg_sgamma import ap_sagr_tut_wing, sagrBaseOptions, sagrAeroDVs
...
test_params = [
    {
        ...
        "ref_file": "jacvecfwd_sgamma_tut_wing.json",
...
class TestJacVecFwdSGAMMA(reg_test_classes.RegTest):
    """
    Tests that given a flow state the FWD (tangent, Tapenade _d) jacobian
    vector products for the SA-noft2-Gamma model agree with the
...
@parameterized_class(test_params)
class TestJacVecFwdSGAMMAFD(reg_test_classes.RegTest):
    """
    ...
    # printing the best-h diagnostics; the CS class (TestJacVecFwdSGAMMACS) is the
...
@parameterized_class(test_params)
class TestJacVecFwdSGAMMACS(reg_test_classes.CmplxRegTest):
    """
    ...
    def cmplx_test_coupling_blocks(self):
        if not hasattr(self, "name"):
            return
        # same keys as TestJacVecFwdSGAMMA.test_coupling_blocks -> the ref file
