...
contains
    ...
    subroutine saSource
        ...
        use inputPhysics, only: useft2SA, useRotationSA, turbProd, equations, &
                    ...
                    SABCM_PG_sensor, SABCM_PG_K, gammaConstant
        use turbUtils, only: smoothMinMax, bcmFlambda
        use inputDiscretization, only: approxSA
        use section, only: sections
        use sa, only: cv13, kar2Inv, cw36, cb3Inv
        use flowvarRefState, only: timeRef, uInf, pInfCorr, rhoInf
        ...
        real(kind=realType) :: nwx, nwy, nwz, Snn, lamL, lam, lamLo, Flam, mlamMax, plamMax, mp
                        ...
                        ! Falkner-Skan similarity map lambda_thetaL(eta*) -> lambda_theta (docs/studies/22_bcm_pressure_gradient/
                ...
                do i = 2, il
                    ...
                    if (use_SABCM) then
                        ...
                        if (SABCM_PG) then
                            ...
                            mp = -SABCM_PG_p
                            if (SABCM_PG_sensor >= 2) then
                                lamMapped = lamL   ! already lambda_theta (no gain/map)
