...
| SA-sγ (novo modelo irmão do GR, γ 1-eq com suavização P&Z): equações, plano de ligação no código, calibração | **fora do repo**: `~/Desktop/Run/MDO_PhD/Transition/sa_sgamma/{00_proposal/RESUMO_FINAL_SA_sgamma.md, 01_implementation_plan/ARCHITECTURE_GR_today_and_SAsGAMMA_plan.md, 02_calibration_check/CALIBRATION_CHECK.md}` → `CORE_BASE/CORE_01_architecture.md` (Part 1, para o que se copia do GR) |
...
| 2D optimisation results / multipoint / GR-vs-BCM optimisation comparison | `OPT_IMPROVEMENTS/OPT_01_2d_tu05_L0_campaign_findings.md` |
...
| [OPT_IMPROVEMENTS/OPT_01_2d_tu05_L0_campaign_findings.md](OPT_IMPROVEMENTS/OPT_01_2d_tu05_L0_campaign_findings.md) | **2D Tu 0.5 % / L0 optimisation campaign findings (2026-09-14→22):** single-point C2 with SA-GR (−49.6 %) vs SA-BCM (−28.6 %) in one design space, multi-Tu and multi-cl C2 (GR robust for free; multipoint does not rescue BCM), the two shape families, the hidden NK stall of the original 2D opt (96/99 solves flagged, pySLSQP ignores it) and the CSANK-only 1e-8 fix, multipoint implementation notes, data/figure/deck index. |
